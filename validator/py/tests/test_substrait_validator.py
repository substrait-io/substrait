...
def test_export_diags():
    ...
    assert lines[0] == 'Warning (plan): encountered values for field(s) not yet understood by the validator: .relations[0]'

    diags = list(sv.plan_to_diagnostics(_BASIC_PLAN))
    for diag in diags:
        assert type(diag) == sv.Diagnostic
    assert diags[0].msg == 'encountered values for field(s) not yet understood by the validator: .relations[0]'
