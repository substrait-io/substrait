...
| Nullability indicator  | A type is either non-nullable or nullable. To express nullability, a question mark is added after the type name (before any parameters). The [`null`](type_classes.md#null-type) type is always nullable and is therefore always written with the suffix, as `null?`. | Optional, defaults to non-nullable    |
