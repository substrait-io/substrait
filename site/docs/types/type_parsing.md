...
| Nullability indicator  | A type is either non-nullable or nullable. To express nullability, a question mark is added after the type name (before any parameters). For nullability to be respected in function definitions, the `nullability` field must also be specified to either `DECLARED_OUTPUT` or `DISCRETE` - see [Nullability Handling](https://substrait.io/expressions/scalar_functions/#nullability-handling)  | Optional, defaults to non-nullable    |
