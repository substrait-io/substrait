...
| FIXEDBINARY&lt;L&gt;          | A binary string of L bytes. When casting, values shorter than L are padded with zeros, and values longer than L are right-trimmed. L must be within [1..2,147,483,647]                                                                                                                                                                                                                                | L-byte `bytes`
...
| INTERVAL_DAY&lt;P&gt;         | Interval day to second. Supports a range of [-3,650,000..3,650,000] days with fractional second precision (P, number of digits) 0 <= P <= 12. Usually stored as separate integers for various components, but only the total elapsed time is significant, i.e. `1d 0s` is considered equal to `0d 86400s`.                                             | `int32` days, `int32` seconds, and an `int64` sub-second component that carries only the fractional part of a second, expressed in 1e(-P) units (e.g. picoseconds when P is 12, so its magnitude stays below 1e12 and fits comfortably in `int64`). Whole days and seconds are held in the `days` and `seconds` fields, not the sub-second field. Each component can never independently specify more than 10,000 years, even if the components have opposite signs (e.g. `3650001d -86400s 0us` is **not** allowed)
| INTERVAL_COMPOUND&lt;P&gt;    | A compound interval type that is composed of elements of the underlying elements and rules of both interval_year and interval_day to express arbitrary durations across multiple grains, with fractional second precision (P, number of digits) 0 <= P <= 12. Substrait gives no definition for the conversion of values between independent grains (e.g. months to days).                                                                |
