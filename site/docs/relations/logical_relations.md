...
A join's output field types are determined by the join type and the input schemas alone. They do not depend on the join expression, nor on anything a consumer can prove about cardinality: an output field that is nullable below remains nullable even where a consumer can show that no null will be produced.
