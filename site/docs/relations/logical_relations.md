...
A join's output field types are determined by the join type and the input schemas alone. 
They are never narrowed by analysis: where the join type makes a field nullable, it stays nullable even if the join condition, the post-join filter, or the data guarantees that no null is produced. A `Left Single` whose condition matches every left record still has a nullable right side.

| Type  | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Output Nullability                                                                        |
| ----- |-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------|
| Inner | Return records from the left side only if they match the right side. Return records from the right side only when they match the left side. For each cross input match, return a record including the data from both sides. Non-matching records are ignored. | Unchanged from both inputs. |
| Outer | Return all records from both the left and right inputs. For each cross input match, return a record including the data from both sides. For any remaining non-match records, return the record from the corresponding input along with nulls for the opposite input. | All fields from both inputs are nullable. |
| Left  | Return all records from the left input. For each cross input match, return a record including the data from both sides. For any remaining non-matching records from the left input, return the left record along with nulls for the right input. | Fields from the left input are unchanged; all fields from the right input are nullable. |
| Right | Same as *Left* except that the right and left inputs are switched. | Fields from the right input are unchanged; all fields from the left input are nullable. |
| Left Semi | Returns records from the left input. These are returned only if the records have a join partner on the right side. | Unchanged. |
| Right Semi | Same as *Left Semi* except that the right and left inputs are switched. | Unchanged. |
| Left Anti  | Return records from the left input. These are returned only if the records do not have a join partner on the right side. | Unchanged. |
| Right Anti | Same as *Left Anti* except that the right and left inputs are switched. | Unchanged. |
| Left Single | Return all records from the left input with no join expansion.  If there exactly one record from the right input matches the left, return the matching record from the right input. For any left records without matching right records, return the left record along with nulls for the right input. If any left records match multiple right records, it is a runtime error. Similar to a left outer join but only returns one right match at most. Useful for nested sub-queries where we need exactly one record in output (or return an error).  See Section 3.2 of https://15721.courses.cs.cmu.edu/spring2018/papers/16-optimizer2/hyperjoins-btw2017.pdf for more information. | Fields from the left input are unchanged; all fields from the right input are nullable. |
| Right Single | Same as *Left Single* except that the right and left inputs are switched. | Fields from the right input are unchanged; all fields from the left input are nullable. |
| Left Mark | Returns one record for each record from the left input.  Appends one additional "mark" column to the output of the join. The new column will be listed after all columns from left side and will be of type nullable boolean.  If there is at least one join partner in the right input where the join condition evaluates to true then the mark column will be set to true.  Otherwise, if there is at least one join partner in the right input where the join condition evaluates to NULL then the mark column will be set to NULL.  Otherwise the mark column will be set to false. | Fields from the left input are unchanged; the mark column is a nullable boolean. |
| Right Mark | Same as *Left Mark* except that the right and left inputs are switched. | Fields from the right input are unchanged; the mark column is a nullable boolean. |
