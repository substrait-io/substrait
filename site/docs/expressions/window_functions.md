...
`offset_expr` may reference the input's fields and must behave as if evaluated once per input record, but it must not contain window or aggregate functions. A null or negative result is invalid; a zero result is equivalent to `CurrentRow`. Because `offset` cannot represent zero, producers should emit `CurrentRow` rather than a zero `offset_expr` so that the bound remains readable by consumers that do not yet support `offset_expr`.
