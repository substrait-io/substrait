...
There are four main types of arguments: value arguments, type arguments, enumerations, and lambda arguments.  Every defined argument must be specified in every invocation of the function.  When specified, the position of these arguments in the function invocation must match the position of the arguments as defined in the YAML function definition.
...
* **Lambda arguments**: arguments that accept lambda expressions for higher-order functions. Lambda arguments are specified as value arguments with a lambda type (e.g., `lambda<(T) -> U>`). The lambda expression defines parameters and a body that can operate on elements of collections. Examples include `array_transform(List<T>, lambda<(T) -> U>) -> List<U>` and `list_filter(List<T>, lambda<(T) -> boolean>) -> List<T>`. See [Lambda Functions](lambda_functions.md) for details.
