...
* **Lambda arguments**: arguments that accept lambda expressions for higher-order functions. Lambda arguments are specified as value arguments with a func type (e.g., `func<(T) -> U>`). The lambda expression defines parameters and a body that can operate on elements of collections. Examples include `transform(List<T>, func<(T) -> U>) -> List<U>` and `filter(List<T>, func<(T) -> boolean>) -> List<T>`. See [Lambda Functions](lambda_functions.md) for details.
