...
def test_unbound_type_parser_accepts_unbound():
    tree, num_errors, errors = parse_type_statement("unbound")
    assert num_errors == 0, errors
    assert tree.getText() == "unbound<EOF>"


def test_unbound_type_parser_rejects_nullable_unbound():
    _tree, num_errors, errors = parse_type_statement("unbound?")
    assert num_errors > 0, errors
